...
struct application_type *
take_a_node(struct application_type **head, struct application_type **prev, struct application_type **temp123) {
    if ((*temp123) == NULL) {
        exit(-1);
    } else if ((*head) == (*prev)) {
...
int main() {
    ...
    while (1) {
        printf("%s",
               "\nEnter command(enrollment , quit , create_application , destroy_application , show_faculties ,back_to_old_data , help ) \n ");
        scanf("%s", command);
        if (strcmp(command, "quit") == 0) {
            puts("Thank you! Goodbye");
            break;
        } else if (strcmp(command, "show_faculties") == 0) {
            show(Specialty);
            ...
            application->total_ball =
                    application->ball1 + application->ball2 + application->ball3 + application->ball_school;
            printf("%s  %s  %s  %d  %d  %d  %d  %s  %s  %d  %d", application->name, application->family_name,
                   application->father_name, application->ball1, application->ball2, application->ball3,
                   application->ball_school, application->passport, application->place, application->school_number,
                   application->total_ball);
            //            application = NULL;
            ...
            FILE *fp3;
            fp3 = fopen("/Users/victoriasviridchik/CLionProjects/2sem_lab6/backup", "r");
            char str_backup[255];
            ...
            printf("back_to_old_data\n");
            printf("save\n");
            printf("enrollment\n");
            printf("help\n");
            printf("Return to the main menu, press esc.  Other way press any key\n");
            int test;
            test = getch();
            if(test==27){
                break;
            }else{
                printf("For more detailed info read doc\n");
            }
